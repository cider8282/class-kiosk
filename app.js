...
function shopTryPurchase(productId){
  ...
  if(idx < 0){ toast("상품을 찾을 수 없어요"); return; }
  ...
  if(lumen < price){ toast("루멘이 부족해요"); return; }
  ...
  // 2) 재고 -1
  products[idx].stock = stock - 1;
  writeJSON(LS.shopProducts, products);

  // 3) 포켓 +1 (슬롯형: 구매 1번 = 1칸)
const imgId = Number.isFinite(p?.imgId) ? p.imgId : 0;
items.push({
  id: crypto.randomUUID(),
  productId: productId,
  name: String(p?.name||"상품"),
  image: String((Number(imgId)%10)+1), // 렌더러에서 숫자 라벨로 표시
  status: "normal"
});
setMyPocketItems(me.id, items);
...
/* === SEBIT FINAL PATCH: student job close -> Firestore immediately + teacher status refresh (2026-05-06) === */
(function(){
  if(window.__sebitJobCloseFinalPatchInstalled) return;
  window.__sebitJobCloseFinalPatchInstalled = true;

  const JOB_STATE = 'jobState';
  let applyingFromCloud = false;
  let directListenerStarted = false;
  ...
  function day(){
    ...
    try{ if(typeof dayKey === 'function') return dayKey(); }catch(_){ }
    const d = new Date();
    const y = d.getFullYear();
    const m = String(d.getMonth()+1).padStart(2,'0');
    const da = String(d.getDate()).padStart(2,'0');
  ...
  function docId(key){ return encodeURIComponent(String(key||'')).replace(/\./g, '%2E'); }
  function doneKey(jobId, sid, d=day()){
    return `sebit_jobdone_${jobId}_${d}${sid ? '_' + String(sid) : ''}`;
  }
  function logKey(jobId, d=day()){
    return `sebit_jobdone_log_${jobId}_${d}`;
  }
  function legacyClosedKey(jobId, d=day()){
    return `sebit_${jobId}_closed_${d}`;
  }
  function isJobCloseKey(key){
    const k = String(key||'');
    return k.startsWith('sebit_jobdone_') || /^sebit_[a-z0-9]+_closed_\d{4}-\d{2}-\d{2}$/.test(k);
  }
  function studentName(sid){
    try{
      const arr = JSON.parse(localStorage.getItem((window.LS && LS.students) || 'sebit:students') || '[]');
      const s = Array.isArray(arr) ? arr.find(x=>String(x && x.id)===String(sid)) : null;
      return s ? String(s.name || sid) : String(sid||'');
    }catch(_){ return String(sid||''); }
  }
  function refreshJobScreens(){
    try{
      const page = String(document.body.getAttribute('data-page') || '');
      if(typeof window.refreshJobPagesFromRealtime === 'function') window.refreshJobPagesFromRealtime();
      if(typeof window.renderStudentShell === 'function' && page.startsWith('student-')) window.renderStudentShell();
      if(typeof window.renderStudentHomeV1 === 'function' && page === 'student-home') window.renderStudentHomeV1();
      const roots = Array.from(document.querySelectorAll('.admin-body, .modal-body, .drawer-body, .jobperf-wrap')).map(el=>{
        const body = el.classList && el.classList.contains('jobperf-wrap') ? el.parentElement : el;
        return body;
      }).filter(Boolean);
      if(typeof window.renderJobPerformanceAdmin === 'function'){
        roots.forEach(root=>{
          const txt = String(root.textContent||'');
          if(txt.includes('직업 수행') || txt.includes('체크리스트') || txt.includes('대기 중') || txt.includes('마감 완료')){
            try{ window.renderJobPerformanceAdmin(root); }catch(_){ }
          }
        });
      }
    }catch(err){ console.warn('[SEBIT JOB CLOSE PATCH] refresh skipped', err); }
  }
  async function pushLocalKeyToFirestore(key){
    if(applyingFromCloud) return;
    if(!hasFirestore()) return;
    if(!isJobCloseKey(key)) return;
    try{
      const raw = localStorage.getItem(key);
      const ref = firebase.firestore().collection(JOB_STATE).doc(docId(key));
      if(raw === null){
        await ref.delete().catch(()=>{});
      }else{
        await ref.set({ key:String(key), raw:String(raw), updatedAt:Date.now(), source:'job-close-patch' }, { merge:false });
      }
      if(typeof window.syncJobKeysToFirestoreNow === 'function'){
        try{ await window.syncJobKeysToFirestoreNow([String(key)]); }catch(_){ }
      }
    }catch(err){ console.error('[SEBIT JOB CLOSE PATCH] key sync failed', key, err); }
  }
  async function pushStudentClose(jobId, sid, closed){
    if(!jobId || !sid) return;
    const d = day();
    const personal = doneKey(jobId, sid, d);
    const whole = doneKey(jobId, '', d);
    const legacy = legacyClosedKey(jobId, d);
    const lkey = logKey(jobId, d);
    let log = {};
    try{ log = JSON.parse(localStorage.getItem(lkey) || '{}') || {}; }catch(_){ log = {}; }
    if(closed){
      localStorage.setItem(personal, '1');
    ...
    // 즉시 서버 반영: setItem 훅에만 맡기지 않고 직접 업로드
    await Promise.all([personal, whole, legacy, lkey].map(pushLocalKeyToFirestore));
    refreshJobScreens();
  }
  async function hydrateJobCloseFromFirestore(reason){
    if(!hasFirestore()) return;
    try{
      const snap = await firebase.firestore().collection(JOB_STATE).get();
      ...
      console.log('[SEBIT JOB CLOSE PATCH] hydrated', reason||'');
      refreshJobScreens();
    }catch(err){
      applyingFromCloud = false;
      console.error('[SEBIT JOB CLOSE PATCH] hydrate failed', err);
    }
  }
  function startDirectJobCloseListener(){
    if(directListenerStarted || !hasFirestore()) return;
    directListenerStarted = true;
    try{
      window.__sebitJobClosePatchUnsub = firebase.firestore().collection(JOB_STATE).onSnapshot(snap=>{
        let changed = false;
        applyingFromCloud = true;
        snap.forEach(doc=>{
          const data = doc.data() || {};
        ...
        applyingFromCloud = false;
        if(changed){
          console.log('[SEBIT JOB CLOSE PATCH] direct jobState updated');
          refreshJobScreens();
        }
      }, err=>console.error('[SEBIT JOB CLOSE PATCH] direct listener failed', err));
    }catch(err){ console.error('[SEBIT JOB CLOSE PATCH] listener install failed', err); }
  }

  // localStorage 직접 변경도 즉시 Firestore에 저장
  try{
    const oldSet = Storage.prototype.setItem;
    const oldRemove = Storage.prototype.removeItem;
    if(!Storage.prototype.__sebitJobClosePatchWrapped){
      Storage.prototype.setItem = function(key, value){
        const ret = oldSet.apply(this, arguments);
        try{ if(this === window.localStorage && isJobCloseKey(key)) setTimeout(()=>pushLocalKeyToFirestore(String(key)), 0); }catch(_){ }
        return ret;
      };
      Storage.prototype.removeItem = function(key){
        const ret = oldRemove.apply(this, arguments);
        try{ if(this === window.localStorage && isJobCloseKey(key)) setTimeout(()=>pushLocalKeyToFirestore(String(key)), 0); }catch(_){ }
        return ret;
      };
      Storage.prototype.__sebitJobClosePatchWrapped = true;
    }
  }catch(err){ console.warn('[SEBIT JOB CLOSE PATCH] storage wrap skipped', err); }

  // 학생 체크리스트 화면의 마감/마감 해제 버튼을 직접 감지
  document.addEventListener('click', function(e){
    const btn = e.target && e.target.closest ? e.target.closest('button') : null;
    if(!btn) return;
    const text = String(btn.textContent || '').replace(/\s+/g,' ').trim();
    if(!text.includes('마감')) return;
    const view = btn.closest('.jobcheck-view-overlay, [data-job-id]');
    const jobId = view?.dataset?.jobId || window.__sebitCurrentStudentJobCtx?.jobId || '';
    const sid = String(window.session?.studentId || '');
    if(!jobId || !sid) return;
    const closed = !(text.includes('마감 해제'));
    // 기존 버튼 동작 후에도 확실히 남도록 여러 번 보강
    setTimeout(()=>pushStudentClose(jobId, sid, closed), 30);
    setTimeout(()=>pushStudentClose(jobId, sid, closed), 300);
    setTimeout(()=>pushStudentClose(jobId, sid, closed), 1000);
  }, true);

  function wrapRenderJobPerformance(){
    if(renderWrapped) return;
    if(typeof window.renderJobPerformanceAdmin !== 'function') return;
    const original = window.renderJobPerformanceAdmin;
    if(original.__sebitJobClosePatchWrapped) return;
    window.renderJobPerformanceAdmin = function(root){
      hydrateJobCloseFromFirestore('before-render');
      return original.apply(this, arguments);
    };
    window.renderJobPerformanceAdmin.__sebitJobClosePatchWrapped = true;
    renderWrapped = true;
  }

  window.sebitHydrateJobCloseNow = hydrateJobCloseFromFirestore;
  window.sebitPushStudentJobCloseNow = pushStudentClose;

  window.addEventListener('load', function(){
    let n=0;
    const t=setInterval(()=>{
      n++;
      startDirectJobCloseListener();
      hydrateJobCloseFromFirestore('startup-'+n);
      wrapRenderJobPerformance();
      if(n>=8) clearInterval(t);
    }, 700);
  });
  document.addEventListener('click', function(e){
    const btn = e.target && e.target.closest ? e.target.closest('[data-admin], [data-go], button') : null;
    const txt = String(btn?.textContent || '');
    const admin = btn?.getAttribute?.('data-admin') || '';
    const go = btn?.getAttribute?.('data-go') || '';
    if(admin === 'job-status' || go.includes('job') || txt.includes('직업 수행') || txt.includes('직업활동')){
      setTimeout(()=>{ startDirectJobCloseListener(); hydrateJobCloseFromFirestore('open-job-status'); wrapRenderJobPerformance(); }, 200);
      setTimeout(()=>refreshJobScreens(), 800);
    }
  }, true);
})();
