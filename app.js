...
let __sebitTeacherAuthUnsub = null;
...
function sebitApplyTeacherAuthCache(auth){
  const normalized = sebitNormalizeTeacherAuth(auth || {});
  if(normalized.passwordHash){
    __sebitTeacherAuthCache = normalized;
    try{ localStorage.setItem(SEBIT_TEACHER_AUTH_CACHE_KEY, normalized.passwordHash); }catch(_){ }
  }
...
function startTeacherAuthRealtimeListener(){
  try{
    if(__sebitTeacherAuthUnsub) return;
    const ref = sebitTeacherAuthRef();
    __sebitTeacherAuthUnsub = ref.onSnapshot(function(snap){
      const exists = (typeof snap.exists === "function") ? snap.exists() : !!snap.exists;
      if(!exists) return;
      sebitApplyTeacherAuthCache(snap.data() || {});
      console.log("[SEBIT] teacher auth realtime updated");
    }, function(err){ console.error("[SEBIT] teacher auth realtime failed", err); });
...
async function checkTeacherPassword(raw){
  ...
  return !!auth?.passwordHash && sebitSimpleHash(pw) === String(auth.passwordHash);
}
